...
static FlightData makeFlightData(uint32_t timestamp, SensorHealth health = SensorHealth::Ok, bool failsafe = false) {
    FlightData data = {};
    data.roll = 10.0f;
    data.pitch = -2.0f;
    data.yaw = 45.0f;
    data.timestamp = timestamp;
    data.sensorHealth = health;
...
void test_estimator_copies_attitude() {
    ...
    EstimatedState state = estimator.update(makeFlightData(1000000), 100.0f, true);
...
void test_estimator_filters_altitude_and_computes_vertical_speed() {
    ...
    estimator.update(makeFlightData(1000000), 100.0f, true);
    EstimatedState state = estimator.update(makeFlightData(2000000), 110.0f, true);
...
void test_estimator_rejects_invalid_sensor_health() {
    ...
    EstimatedState state = estimator.update(makeFlightData(1000000, SensorHealth::Stale), 100.0f, true);
...
void test_estimator_handles_missing_baro() {
    ...
    EstimatedState state = estimator.update(makeFlightData(1000000), 0.0f, false);
