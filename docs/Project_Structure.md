...
- Sensor DMA orkestrasyonu `drivers/sensors/SensorDmaBus.*` icindedir. MPU ve yardimci sensor okumalari RX+TX DMA komut zinciri kullanir; kanal tahsis hatasinda alinmis kanal geri birakilir. GY87 mag/baro state machine'i `SensorAuxBus.*` icine ayrildi ve `Sensors.cpp` 327 satira indirildi.
...
- `Scheduler` ve `PreflightHealth` cekirdek siniflari test-first eklendi. Core0 sensor/RC/state/preflight/watchdog frekanslari scheduler'a baglandi; telemetry ve blackbox 50Hz tasiyici task uzerinde runtime rate gate kullanir. Core1 control loop deterministik 400Hz task olarak kalir.
...
- `estimators/` altinda `ComplementaryEstimator`, hizli `BaroAltitudeEstimator` ve hafif `EkfLiteEstimator` bulunur. EKF-lite runtime `SensorPipeline` akimina baglidir; 1D altitude/vertical-speed, 2x2 kovaryans ve baro spike gating kullanir. Estimator katmani buyuk `FlightData` yerine dar `EstimatorInput` tipini tuketir.
