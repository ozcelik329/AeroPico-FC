...
bool paramSlotMatches(uint32_t offset, const ParamStorageBlob& expected) {
    const auto* stored = reinterpret_cast<const ParamStorageBlob*>(XIP_BASE + offset);
    const ParamStorageBlob candidate = *stored;
    return ParamStorage::hasValidEnvelope(candidate) &&
           candidate.generation == expected.generation &&
           memcmp(&candidate, &expected, sizeof(ParamStorageBlob)) == 0;
}
}
#endif
...
bool RPFlashParamStorage::save(const ParamStorageBlob& blob) {
    ...
    for (uint8_t attempt = 0; attempt < FLASH_WRITE_ATTEMPTS; ++attempt) {
        if (RPFlashWriter::execute(programParamFlash, &context) &&
            paramSlotMatches(context.offset, journalBlob)) {
            return true;
        }
        delay(5);
    }
