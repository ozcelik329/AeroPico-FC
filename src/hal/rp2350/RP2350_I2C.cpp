...
bool RP2350I2C::writeRaw(uint8_t address, const uint8_t* data, size_t length, bool nostop) {
    ...
    return i2c_write_timeout_us(_instance, address, data, length, nostop, I2C_BLOCKING_TIMEOUT_US) == (int)length;
}
...
bool RP2350I2C::readRaw(uint8_t address, uint8_t* data, size_t length, bool nostop) {
    ...
    return i2c_read_timeout_us(_instance, address, data, length, nostop, I2C_BLOCKING_TIMEOUT_US) == (int)length;
}

bool RP2350I2C::probeAddress(uint8_t address, uint timeoutUs) {
    uint8_t value = 0;
    return i2c_read_timeout_us(_instance, address, &value, 1, false, timeoutUs) == 1;
}
