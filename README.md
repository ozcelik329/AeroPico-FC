...
* **MAVLink over USB for bench testing** — Mission Planner, QGroundControl, AeroPico Configurator and future AeroPico GCS can use the same MAVLink Common transport.
...
AeroPico-FC v1.0 RCI targets a well-behaved generic MAVLink autopilot profile:

- Mission Planner: heartbeat, arm/disarm command ACK, VFR HUD, SYS_STATUS, parameters, GPS no-fix/no-GPS status and mission-count stub.
- QGroundControl: same MAVLink Common telemetry and command path.
- AeroPico Configurator / future AeroPico GCS: use the same transport and safe command gates.

Bench connection should start with Pico USB Serial. The ESP32 bridge in `Esp32Cam/`
is optional and forwards MAVLink UART to UDP 14550 after separate bench validation.

ArduPilot-specific setup wizards and mode names are not emulated. The target is a
clean generic fixed-wing FC, not an ArduPilot clone.
...
| Mission Planner / QGC generic MAVLink | ✅ |
